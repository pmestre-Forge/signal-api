...
def channel_web_view(name: str):
    ...
    html = (
        ...
        '</div>'
        '<script>'
        'var refreshTimer=setInterval(function(){location.reload()},10000);'
        'var inp=document.getElementById("msg");'
        'inp.addEventListener("focus",function(){clearInterval(refreshTimer)});'
        'inp.addEventListener("blur",function(){if(!inp.value){refreshTimer=setInterval(function(){location.reload()},10000)}});'
        'function sendMsg(){'
        '  var msg=inp.value;'
        '  if(!msg)return;'
